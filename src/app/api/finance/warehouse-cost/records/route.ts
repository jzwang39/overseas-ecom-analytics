...
async function loadRecordDataByIds(ids: number[]) {
  ...
  const [rows] = await pool.query<
    (RowDataPacket & {
      warehouse_cost_record_id: number;
      field_key: string;
      field_value: string | null;
    })[]
  >(
    `
    SELECT warehouse_cost_record_id, field_key, field_value
    FROM warehouse_cost_record_fields
    WHERE warehouse_cost_record_id IN (${placeholders})
  `,
    ids,
  );
  ...
  for (const r of rows) {
    const obj = out.get(r.warehouse_cost_record_id);
    if (!obj) continue;
  ...
  if (emptyKeys.length) {
    for (const batch of chunkArray(Array.from(new Set(emptyKeys)), 200)) {
      ...
      await conn.query<ResultSetHeader>(
        `DELETE FROM warehouse_cost_record_fields WHERE warehouse_cost_record_id = ? AND field_key IN (${placeholders})`,
        [recordId, ...batch],
      );
    }
  }
...
export async function GET(req: NextRequest) {
  ...
  const fields = await getWarehouseCostFields();
  ...
  if (q) {
    where.push(
      ...
      EXISTS (
        SELECT 1
        FROM warehouse_cost_record_fields kv
        WHERE kv.warehouse_cost_record_id = r.id
...
export async function POST(req: NextRequest) {
  ...
  if ("action" in parsed.data && parsed.data.action === "batch_copy_yesterday") {
    const action = "warehouse_cost.batch_copy_yesterday";
    ...
    try {
      ...
      const [result] = await conn.query<ResultSetHeader>(
        `INSERT INTO warehouse_cost_records() VALUES ${insertPlaceholders}`,
      );
      ...
      if (keepFields.length) {
        ...
        >(
          ...
          SELECT warehouse_cost_record_id, field_key, field_value
          FROM warehouse_cost_record_fields
          WHERE warehouse_cost_record_id IN (${idPlaceholders})
        ...
        for (const r of kvRows) {
          const m = byOldId.get(r.warehouse_cost_record_id);
          if (!m) continue;
      ...
      await logOperation({
        ...
        action,
        targetType: "warehouse_cost_record",
        targetId: null,
  ...
  try {
    await conn.beginTransaction();
    const [result] = await conn.query<ResultSetHeader>("INSERT INTO warehouse_cost_records() VALUES ()");
