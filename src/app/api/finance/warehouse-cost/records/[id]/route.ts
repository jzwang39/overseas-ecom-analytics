...
export async function PATCH(
  ...
  try {
    ...
    const [existing] = await conn.query<(RowDataPacket & { id: number })[]>(
      "SELECT id FROM warehouse_cost_records WHERE id = ? LIMIT 1",
      [recordId],
    );
    ...
    await conn.query<ResultSetHeader>("UPDATE warehouse_cost_records SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", [
      recordId,
    ]);
