...
async function loadRecordDataByIds(ids: number[]) {
  ...
  const [rows] = await pool.query<
    (RowDataPacket & {
      penalty_amount_record_id: number;
      field_key: string;
      field_value: string | null;
    })[]
  >(
    `
    SELECT penalty_amount_record_id, field_key, field_value
    FROM penalty_amount_record_fields
    WHERE penalty_amount_record_id IN (${placeholders})
  `,
    ids,
  );
  ...
  for (const r of rows) {
    const obj = out.get(r.penalty_amount_record_id);
    if (!obj) continue;
...
export async function GET(req: NextRequest) {
  ...
  if (q) {
    where.push(
      ...
      EXISTS (
        SELECT 1
        FROM penalty_amount_record_fields kv
        WHERE kv.penalty_amount_record_id = r.id
...
export async function POST(req: NextRequest) {
  ...
  if ("action" in parsed.data && parsed.data.action === "batch_copy_yesterday") {
    const action = "penalty_amount.batch_copy_yesterday";
    ...
    try {
      ...
      const [result] = await conn.query<ResultSetHeader>(
        `INSERT INTO penalty_amount_records() VALUES ${insertPlaceholders}`,
      );
      ...
      if (keepFields.length) {
        ...
        >(
          ...
          SELECT penalty_amount_record_id, field_key, field_value
          FROM penalty_amount_record_fields
          WHERE penalty_amount_record_id IN (${idPlaceholders})
        ...
        for (const r of kvRows) {
          const m = byOldId.get(r.penalty_amount_record_id);
          if (!m) continue;
  ...
  try {
    await conn.beginTransaction();
    const [result] = await conn.query<ResultSetHeader>("INSERT INTO penalty_amount_records() VALUES ()");
