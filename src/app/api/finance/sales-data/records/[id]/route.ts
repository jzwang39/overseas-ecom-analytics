...
export async function PATCH(
  ...
  try {
    ...
    const [existing] = await conn.query<(RowDataPacket & { id: number })[]>(
      "SELECT id FROM sales_data_records WHERE id = ? LIMIT 1",
      [recordId],
    );
    ...
    await conn.query<ResultSetHeader>("UPDATE sales_data_records SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", [
      recordId,
    ]);
