...
const RECORD_TYPE = "sales_data";

const TEMU_SALES_DATA_FIELDS: string[] = [
  "结算流水ID",
  "批次号",
  "PO单号",
  "单据号",
...
async function loadRecordDataByIds(ids: number[]) {
  ...
  const [rows] = await pool.query<
    (RowDataPacket & {
      sales_data_record_id: number;
      field_key: string;
      field_value: string | null;
    })[]
  >(
    `
    SELECT sales_data_record_id, field_key, field_value
    FROM sales_data_record_fields
    WHERE sales_data_record_id IN (${placeholders})
  `,
    ids,
  );
  ...
  for (const r of rows) {
    const obj = out.get(r.sales_data_record_id);
    if (!obj) continue;
  ...
  if (emptyKeys.length) {
    for (const batch of chunkArray(Array.from(new Set(emptyKeys)), 200)) {
      ...
      await conn.query<ResultSetHeader>(
        `DELETE FROM sales_data_record_fields WHERE sales_data_record_id = ? AND field_key IN (${placeholders})`,
        [recordId, ...batch],
      );
    }
  }
...
export async function GET(req: NextRequest) {
  ...
  const fields = await getSalesDataFields();
  ...
  if (q) {
    where.push(
      ...
      EXISTS (
        SELECT 1
        FROM sales_data_record_fields kv
        WHERE kv.sales_data_record_id = r.id
...
export async function POST(req: NextRequest) {
  ...
  if ("action" in parsed.data && parsed.data.action === "batch_copy_yesterday") {
    const action = "sales_data.batch_copy_yesterday";
    ...
    const keepFields = Array.from(
      new Set(parsed.data.keepFields.map((f) => f.trim()).filter((f) => f && allowedFields.has(f))),
    );
    const conn = await pool.getConnection();
    try {
      await conn.beginTransaction();
      const insertPlaceholders = oldIds.map(() => "()").join(", ");
      const [result] = await conn.query<ResultSetHeader>(`INSERT INTO sales_data_records() VALUES ${insertPlaceholders}`);
      ...
      if (keepFields.length) {
        ...
        >(
          ...
          SELECT sales_data_record_id, field_key, field_value
          FROM sales_data_record_fields
          WHERE sales_data_record_id IN (${idPlaceholders})
        ...
        for (const r of kvRows) {
          const m = byOldId.get(r.sales_data_record_id);
          if (!m) continue;
  ...
  try {
    await conn.beginTransaction();
    const [result] = await conn.query<ResultSetHeader>("INSERT INTO sales_data_records() VALUES ()");
