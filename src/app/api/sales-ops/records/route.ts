...
export async function POST(req: NextRequest) {
  ...
  if ("action" in parsed.data && parsed.data.action === "batch_copy_yesterday") {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO sales_ops_records(data) VALUES ${placeholders}`,
      params,
    );
  ...
  const [result] = await pool.query<ResultSetHeader>("INSERT INTO sales_ops_records(data) VALUES (CAST(? AS JSON))", [
